...
namespace Tjs2.Engine
{
    ...
    public class InterCodeObject : CustomObject, SourceCodeAccessor
    {
        ...
        internal ScriptBlock mBlock;

        internal InterCodeObject mParent;

        internal string mName;

        internal int mContextType;

        internal short[] mCode;

        internal Variant[] mDataArray;

        /// <summary>引数の数</summary>
        internal int mMaxVariableCount;

        /// <summary>予约领域(this,proxy用)</summary>
        internal int mVariableReserveCount;

        /// <summary>关数内レジスタ数(ローカル变数の数)</summary>
        internal int mMaxFrameCount;

        internal int mFuncDeclArgCount;

        internal int mFuncDeclUnnamedArgArrayBase;

        internal int mFuncDeclCollapseBase;
        ...
        {
            var s = base.ToString();
            if (string.IsNullOrWhiteSpace(s) || s.Equals("empty"))
            {
                s = mName;
            }
            return s;
