...
namespace Tjs2.Engine
{
    ...
    public class InterCodeGenerator : SourceCodeAccessor
    {
        ...
        public virtual void CrateIfExprCode(ExprNode node)
        {
            ...
            int resaddr = GenNodeCode(fr, node, RT_NEEDED | RT_CFLAG, 0, new InterCodeGenerator.SubParam
                ());
            int nodepos = (node != null ? node.Position : -1);
            bool inv = false;
            if (!(resaddr == GNC_CFLAG || resaddr == GNC_CFLAG_I))
            {
                //putCode(VM_TT, nodepos );
        ...
        public virtual void EnterBlock()
        {
            ...
            mNestVector.AddItem(new NestData());
        ...
        public virtual ExprNode MakeConstValNode(Variant val)
        {
            ...
            node.OpCode = (Token.T_CONSTVAL);
            node.SetValue(val);
            node.Position = (GetLexPos());
            return node;
        }
        ...
        {
            // 定数の最适化
            if (node1 != null && node1.OpCode == Token.T_CONSTVAL)
            {
                ExprNode ret = null;
                switch (opecode)
                {
        ...
        {
            // 定数の最适化
            if (node1 != null && node1.OpCode == Token.T_CONSTVAL && node2 != null && node2
                .OpCode == Token.T_CONSTVAL)
            {
                switch (opecode)
                {
                    case Token.T_COMMA:
            ...
            , InterCodeGenerator.SubParam param)
        {
            ...
            int resaddr;
            int node_pos = (node != null ? node.Position : -1);
            switch (node.OpCode)
            {
                ...
                case Token.T_LOGICALAND:
                    {
                        ...
                        mCodeArea[mCodeAreaPos] = (short)(node.OpCode == Token.T_LOGICALOR ? (inv ?
                ...
                case Token.T_TYPEOF:
                    {
                        ...
                        if (cnode.OpCode == Token.T_DOT || cnode.OpCode == Token.T_LBRACKET ||
                             cnode.OpCode == Token.T_WITHDOT)
                        {
                            haspropnode = true;
                        }
                        else
                ...
                case Token.T_NEW:
                    {
                        ...
                        ExprNode cnode = node.GetNode(0);
                        if (node.OpCode == Token.T_LPARENTHESIS && (cnode.OpCode == Token.T_DOT
                             || cnode.OpCode == Token.T_LBRACKET))
                        {
                            haspropnode = true;
                        }
                        else
                        ...
                        if (node.OpCode == Token.T_LPARENTHESIS && cnode.OpCode == Token.T_SYMBOL)
                        {
                        ...
                        try
                        {
                            ...
                            else
                            {
                                ...
                                mCodeArea[mCodeAreaPos] = (short)(node.OpCode == Token.T_NEW ? VM_NEW : VM_CALL
                                    );
                                mCodeAreaPos++;
                ...
                case Token.T_LBRACKET:
                    {
                        ...
                        bool direct = node.OpCode == Token.T_DOT;
                        int dp;
                        InterCodeGenerator.SubParam param2 = new InterCodeGenerator.SubParam();
                        param2.mSubType = stNone;
                        resaddr = GenNodeCode(frame, node.GetNode(0), RT_NEEDED, 0, param2);
                            ...
                            case stIgnorePropSet:
                                {
                                    ...
                                    {
                                        if (node.GetNode(0).OpCode == Token.T_THIS_PROXY)
                                        {
                ...
                case Token.T_SYMBOL:
                    {
                        ...
                        else
                        {
                            ...
                            nodep.OpCode = (Token.T_DOT);
                            nodep.Position = (node_pos);
                            ExprNode node1 = new ExprNode();
                            mNodeToDeleteVector.AddItem(node1);
                            nodep.Add(node1);
                            node1.OpCode = (mAsGlobalContextMode ? Token.T_GLOBAL : Token.T_THIS_PROXY);
                            node1.Position = (node_pos);
                            ExprNode node2 = new ExprNode();
                            mNodeToDeleteVector.AddItem(node2);
                            nodep.Add(node2);
                            node2.OpCode = (Token.T_SYMBOL);
                            node2.Position = (node_pos);
                ...
                case Token.T_PROPACCESS:
                    {
                        // unary '&' operator
                        // unary '*' operator
                        if (node.OpCode == (Tjs.mUnaryAsteriskIgnoresPropAccess ? Token.T_PROPACCESS
                             : Token.T_IGNOREPROP))
                        {
                            // unary '&' operator
                ...
                case Token.T_WITHDOT:
                    {
                        ...
                        mNodeToDeleteVector.AddItem(node1);
                        nodep.Add(node1);
                        node1.OpCode = (Token.T_WITHDOT_PROXY);
                        node1.Position = (node_pos);
